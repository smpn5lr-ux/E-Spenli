...
// Component for Attendance Weight Settings
function AttendanceWeightSettings() {
  ...
  const handleInputChange = (field: keyof typeof weights, value: string) => {
    ...
    if (numValue === '' || (!isNaN(numValue as number) && numValue >= 0)) {
  ...
  const handleSave = async () => {
    ...
    try {
      const finalWeights = Object.fromEntries(
        Object.entries(weights).map(([key, value]) => [key, isNaN(Number(value)) ? 0 : Number(value)])
      );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Atur bobot nilai untuk setiap status kehadiran.</CardDescription>
      </CardHeader>
      <CardContent>
        {isLoading ? (
      ...
      <CardFooter className="border-t p-4 sm:p-6">
        <Button onClick={handleSave} disabled={isSaving || isLoading}>
          {isSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Simpan Bobot
        </Button>
      </CardFooter>
    </Card>
  );
...
// Component for Monthly Holiday Settings
const daysOfWeek = [
  ...
  return (
    <Card>
        <CardHeader>
            ...
            <CardDescription>Tandai hari libur spesifik (misal: libur nasional) untuk bulan ini.</CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-1 space-y-4 border-l-0 md:border-l md:pl-6">
                <h3 className="font-semibold">Ringkasan Konfigurasi</h3>
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex items-center h-10 w-full rounded-md border border-input bg-muted px-3 py-2 text-sm select-none">{isMonthlyConfigLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : calculatedWorkDays} hari</div>
                     <p className="text-xs text-muted-foreground">Total setelah dikurangi libur rutin & spesifik.</p>
                </div>
            </div>
        </CardContent>
         <CardFooter className="border-t p-4 sm:p-6">
            <Button onClick={handleSave} disabled={isSaving || isMonthlyConfigLoading}>{isSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Simpan Bulan Ini</Button>
        </CardFooter>
    </Card>
  );
}
...
export default function KonfigurasiAbsenPage() {
  ...
  // Effect to populate form from Firestore data
  useEffect(() => {
    if (schoolConfigData) {
      ...
      if (schoolConfigData.checkOutTimes) {
          ...
          setCheckOutTimes({
              1: { start: oldStart, end: oldEnd }, 2: { start: oldStart, end: oldEnd }, 3: { start: oldStart, end: oldEnd },
              4: { start: oldStart, end: oldEnd }, 5: { start: oldStart, end: oldEnd }, 6: { start: oldStart, end: oldEnd },
          });
      }
  ...
  // Effect to generate QR code from its value
  useEffect(() => {
    if (qrCodeValue) {
      setIsQrLoading(true);
  ...
  // CORE FEATURE: QR Code Download Functionality
  const downloadQRCode = async (format: 'png' | 'pdf') => {
    if (!qrCodeDataUrl) {
      toast({ variant: 'destructive', title: 'Gagal Mengunduh', description: 'QR Code belum siap.' });
      return;
    }
    if (format === 'png') {
    ...
    toast({ title: 'Berhasil', description: `QR Code diunduh sebagai ${format.toUpperCase()}.` });
  };
  ...
  const handleGenerateNewQr = () => {
    ...
    updateDocumentNonBlocking(schoolConfigRef, { qrCodeValue: newQrValue });
    setQrCodeValue(newQrValue);
    toast({ title: 'QR Code Diperbarui', description: 'QR Code baru telah dibuat.' });
  };
  ...
  const handleSave = () => {
    ...
    const configUpdate = {
      ...
      checkOutStartTime: checkOutTimes[1].start,
      checkOutEndTime: checkOutTimes[1].end,
    };
    setDocumentNonBlocking(schoolConfigRef, configUpdate, { merge: true });

    const legacyUpdate = {
        jamMasukMulai: checkInStart, jamMasukSelesai: checkInEnd,
        jamPulangMulai: checkOutTimes[1].start, jamPulangSelesai: checkOutTimes[1].end,
    };
    setDocumentNonBlocking(legacyConfigRef, legacyUpdate, { merge: true });

    toast({ title: 'Pengaturan Disimpan', description: 'Konfigurasi absensi telah diperbarui.' });
    setIsSaving(false);
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-1">
          ...
          <CardContent className="flex flex-col items-center justify-center gap-4 p-6">
            <div className="p-4 border rounded-lg bg-white aspect-square w-full max-w-[256px]">
            ...
            <Button onClick={handleGenerateNewQr} variant="outline" className="w-full max-w-[256px]" disabled={isQrLoading}>{isQrLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCw className="mr-2 h-4 w-4" />}Buat QR Baru</Button>
          </CardContent>
          {/* CORE FEATURE: DOWNLOAD BUTTONS */}
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Atur parameter utama untuk sistem absensi.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6 p-6">
            {/* Holiday Mode */}
            <div className="rounded-lg border p-4 space-y-4">
                <div className="flex items-center justify-between">
                    <div><Label htmlFor="holiday-mode" className="font-semibold">Mode Libur</Label><p className="text-sm text-muted-foreground">Non-aktifkan sistem absensi untuk semua.</p></div>
            ...
            <div className="rounded-lg border p-4 space-y-4">
              <div className="flex items-center justify-between">
                <div><Label htmlFor="use-location" className="font-semibold">Validasi Lokasi</Label><p className="text-sm text-muted-foreground">Wajibkan pengguna berada di area sekolah.</p></div>
                <Switch id="use-location" checked={useLocationValidation} onCheckedChange={setUseLocationValidation} disabled={holidayMode} />
              </div>
              {useLocationValidation && (
                <div className="space-y-4 pt-4 border-t">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between gap-4"><Label>Koordinat Sekolah</Label><Button type="button" variant="outline" size="sm" onClick={handleGetCurrentLocation} disabled={isLocating || holidayMode}>{isLocating ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <LocateFixed className="mr-2 h-4 w-4" />}Dapatkan Lokasi</Button></div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div><Input id="latitude" type="text" value={latitude} onChange={(e) => setLatitude(e.target.value)} placeholder="Latitude" disabled={holidayMode || isLocating} /></div>
                      <div><Input id="longitude" type="text" value={longitude} onChange={(e) => setLongitude(e.target.value)} placeholder="Longitude" disabled={holidayMode || isLocating} /></div>
                    </div>
                  </div>
                  <div className="space-y-2"><Label htmlFor="radius">Radius Valid (meter)</Label><Input id="radius" type="number" value={radius} onChange={(e) => setRadius(Number(e.target.value))} placeholder="100" disabled={holidayMode} /></div>
                  {/* CORE FEATURE: MAP PREVIEW */}
                  <div className="space-y-2">
                    <Label>Pratinjau Peta</Label>
                    <div className="aspect-video w-full overflow-hidden rounded-lg border">
                      <iframe key={`${latitude}-${longitude}`} width="100%" height="100%" loading="lazy" allowFullScreen referrerPolicy="no-referrer-when-downgrade" src={`https://maps.google.com/maps?q=${latitude},${longitude}&hl=id&z=15&output=embed`} title="Pratinjau Lokasi Peta"></iframe>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="rounded-lg border p-4 space-y-4">
                <div className="flex items-center justify-between">
                    <div><Label htmlFor="use-time" className="font-semibold">Validasi Jam Kerja</Label><p className="text-sm text-muted-foreground">Wajibkan absen di jam yang ditentukan.</p></div>
                            ...
                            <div className="space-y-3 rounded-md border p-3">
                                {daysOfWeek.filter(d => d.value !== 0).map(day => (
                                    <div key={day.value} className="grid grid-cols-1 sm:grid-cols-5 items-center gap-2">
                                        <Label className="sm:col-span-2 text-sm font-normal">{day.label}</Label>
                                        <div className="sm:col-span-3 grid grid-cols-2 gap-2">
                                            <Input type="time" value={checkOutTimes[day.value as keyof typeof checkOutTimes]?.start || ''} onChange={e => handleTimeChange(day.value, 'start', e.target.value)} disabled={holidayMode} />
                                            <Input type="time" value={checkOutTimes[day.value as keyof typeof checkOutTimes]?.end || ''} onChange={e => handleTimeChange(day.value, 'end', e.target.value)} disabled={holidayMode} />
                                        </div>
                                    </div>
                                ))}
                            </div>
           ...
           <CardFooter className="border-t p-4 sm:p-6">
             <Button onClick={handleSave} disabled={isSaving}>{isSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Simpan Pengaturan</Button>
          </CardFooter>
        </Card>
      </div>

      {/* SECOND ROW: MONTHLY CALENDAR, WEIGHTS */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
